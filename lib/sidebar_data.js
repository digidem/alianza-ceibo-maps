...
function getHomeData (data, intl, show) {
  ...
  const list = Object.keys(nations).map(key => {
    ...
    const intlName = (intl.locale === 'es' && nation.properties['Nombre Español']) || nation.properties._nationName
    return {
      id: nation.id,
      intlName: intlName,
      name: nation.properties._nationName,
      solar: nation.stats.solar,
      water: nation.stats.water,
      waterRequired: nation.stats.waterRequired,
      color: nation.properties._color,
      image: getPhoto(nation)
    }
  })

  return {
    intlTitle: intl.formatMessage(show === 'agua' ? messages.aguaTitle : messages.homeTitle),
    image: '/sidebar.jpg',
    ...
    list: list.sort(function (a, b) { return b.water - a.water })
  }
}
...
function getNationData (data, intl, nationName) {
  ...
  const intlTitle = (intl.locale === 'es' && nation.properties['Nombre Español']) || nation.properties._nationName
  ...
  return {
    ...
    water: stats.water,
    waterRequired: stats.waterRequired,
    listTitle: intlTitle + ' Communities',
    list: list
  }
}
...
function getCommunityData (data, intl, nationName, communityName) {
  ...
  return {
    intlTitle: community.properties._communityName,
...
function getPhoto (feature) {
  return feature &&
    ...
    feature.properties.Foto[0].thumbnails &&
