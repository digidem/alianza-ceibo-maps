...
function transformData (featureCollections) {
  ...
  areaGeometries.features.forEach(function (feature) {
    ...
    data.areas[fProps._areaName] = feature
    data.byId[feature.id] = feature
...
function addIconFieldAndFilter (f) {
  var icon
  var programas = f.properties[PROGRAM_PROP] || []
  if (includes(programas, WATER_PROGRAM) && includes(programas, SOLAR_PROGRAM)) {
    icon = 'comunidad-agua-solar'
  } else if (includes(programas, WATER_PROGRAM)) {
    icon = 'comunidad-agua'
  } else if (includes(programas, SOLAR_PROGRAM)) {
