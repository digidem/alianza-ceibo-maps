...
const WATER_REQUIRED_PROP = 'Agua Requerido'
...
function fetchData (show, cb) {
  xhr.get('/data.json', { headers: {
    ...
    cb(null, transformData(featureCollections, show))
  })
}

function transformData (featureCollections, show) {
  ...
  Object.keys(featureCollections).forEach(function (key) {
    ...
    fc.features.forEach(function (feature) {
      ...
      if (key === COMMUNITIES_KEY) {
        ...
        // Filter communities without programs or story
        const hasPrograms = fProps[PROGRAM_PROP] && fProps[PROGRAM_PROP].length
        const hasStories = fProps.Historias && fProps.Historias.length
        if (!hasPrograms && !hasStories) return
        // Filter communities without stories and without any installations
        if (!hasStories && !fProps[SOLAR_COUNT_PROP] && !fProps[WATER_COUNT_PROP] && !fProps[WATER_REQUIRED_PROP]) return
        // Filter communities with water installations in progress when on maps that aren't agua only
        if (show !== 'agua' && fProps[WATER_REQUIRED_PROP]) return
  ...
  Object.keys(data.nations).forEach(function (nation) {
    ...
    data.nations[nation].communities = communitiesIds.reduce(function (acc, id) {
      ...
      community.stats = {
        ...
        waterRequired: community.properties[WATER_REQUIRED_PROP] || 0
      }
      stats.solar += community.stats.solar
      stats.water += community.stats.water
      stats.waterRequired += community.stats.waterRequired
      return acc
    }, {})
    ...
    data.stats.waterRequired += stats.waterRequired
  ...
  Object.keys(data.areas).forEach(function (areaName) {
    ...
    areaFeature.communities = communitiesIds.reduce(function (acc, id) {
      ...
      stats.waterRequired += (community.properties[WATER_REQUIRED_PROP] || 0)
      return acc
    }, {})
...
function addIconFieldAndFilter (f) {
  ...
  const hasWater = includes(programas, WATER_PROGRAM) && f.properties[WATER_COUNT_PROP] > 0
  const hasSolar = includes(programas, SOLAR_PROGRAM) && f.properties[SOLAR_COUNT_PROP] > 0
  const hasWaterRequired = f.properties[WATER_REQUIRED_PROP] > 0
  ...
  } else if (hasWaterRequired) {
    icon = 'comunidad-agua-requerido'
  } else {
    icon = 'comunidad'
  }
